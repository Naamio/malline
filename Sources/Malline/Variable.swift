...
class FilterExpression : Resolvable {
    ...
    init(token: String, parser: TokenParser) throws {
        ...
        if bits.isEmpty {
            filters = []
            variable = Variable("")
            throw StencilSyntaxError("Variable tags must include at least 1 argument")
        }
...
/// A structure used to represent a stencil variable, and to resolve it in a given context.
public struct Variable : Equatable, Resolvable {
    ...
    public func resolve(_ context: Context) throws -> Any? {
        ...
        } else if let tag = current as? TagType {
            current = try tag.render(context)
        }
...
public struct RangeVariable: Resolvable {
    ...
    public func resolve(_ context: Context) throws -> Any? {
        ...
        guard let from = fromResolved.flatMap(toNumber(value:)).flatMap(Int.init) else {
            throw StencilSyntaxError("'from' value is not an Integer (\(fromResolved ?? "nil"))")
        }
        
        guard let to = toResolved.flatMap(toNumber(value:)).flatMap(Int.init) else {
            throw StencilSyntaxError("'to' value is not an Integer (\(toResolved ?? "nil") )")
        }
    ...
    func normalize() -> Any? {
        ...
        for (key, value) in self {
            if let key = key as? String {
                dictionary[key] = Malline.normalize(value)
            } else if let key = key as? CustomStringConvertible {
                dictionary[key.description] = Malline.normalize(value)
            }
        }
